...
import { OtpGenerator, ReqRegister, UpdateInfoSocial } from '~/entities';
...
export class AuthService {
  ...
  async register(req: ReqRegister, dataRegister: RegisterInput): Promise<any> {
    ...
    const userRegister: CreateUserInput = {
      email,
      name,
      password,
  ...
  async login(dataLogin: LoginInput): Promise<any> {
  ...
  async adminLogin(dataLogin: LoginInput): Promise<any> {
    const { password } = dataLogin;
    let { email } = dataLogin;
    email = email.trim().toLowerCase();
  ...
  async forgotPassword(forgotPasswordInput: ForgotPasswordInput): Promise<any> {
    let { email } = forgotPasswordInput;
    email = email.trim().toLowerCase();
